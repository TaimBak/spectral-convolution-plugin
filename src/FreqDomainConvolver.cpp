...
FreqDomainConvolver::FreqDomainConvolver(const std::vector<float> &h, int fftOrder = 10, int blockSize = 128)
    : fftOrder(fftOrder)
    , K(1 << fftOrder)
    , B(blockSize)
    , N((int)h.size())
    ...
    , 0.0f)
{
    ...
    for (int n = 0; n < K; ++n)
        timeC[n] = (n < N ? juce::dsp::Complex<float>(h[n], 0.0f) : juce::dsp::Complex<float>(0.0f, 0.0f));
...
std::vector<float> FreqDomainConvolver::processBlock(const float *x, int numSamples)
{
    ...
    // newOverlap[t] = overlap[t + head] + (timeC[head + t].real() if t < tail
    // else 0)
